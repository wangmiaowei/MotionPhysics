...
def decode_param_json(json_file):
    ...
    if "downsample_scale" in sim_params.keys():
        material_params["downsample_scale"] = sim_params["downsample_scale"]
    else:
        material_params["downsample_scale"] = 1.0
    ...
    if "E" in sim_params.keys():
        ...
        material_params["E"] = 1e-2
    
    # add 
    if "mu" in sim_params.keys():
        material_params["mu"] = sim_params["mu"]
    ...
    if "bulk" in sim_params.keys():
        material_params["bulk"] = sim_params["bulk"]
    ...
    # time step
    time_params = {}
    if "substep_perframe" in sim_params.keys():
        time_params["substep_perframe"] = sim_params["substep_perframe"]
    else:
        time_params["substep_perframe"] = 256

    if "total_time" in sim_params.keys():
        time_params["total_time"] = sim_params["total_time"]
    else:
        time_params["total_time"] = 3
    ...
    optimize_params = {}
    if "prompt" in sim_params.keys():
        optimize_params["prompt"] = sim_params["prompt"]
...
def set_boundary_conditions(
    mpm_solver: MPM_Simulator_WARP, bc_params: dict, time_params: dict,freeze_pts=None,sim_pos=None
):    
    ...
    force_info = []
    if freeze_pts is not None:

        device = freeze_pts.device
        grid_size = mpm_solver.mpm_model.n_grid

        freeze_idx = (freeze_pts * mpm_solver.mpm_model.inv_dx).long()
        freeze_idx = freeze_idx.clamp(min=0, max=grid_size - 1)

 
        total_grid_pts = grid_size * grid_size * grid_size
        flat_counts = torch.zeros(total_grid_pts, dtype=torch.int32, device=device)


        indices = freeze_idx[:, 0] * (grid_size * grid_size) + freeze_idx[:, 1] * grid_size + freeze_idx[:, 2]
        ones = torch.ones_like(indices, dtype=torch.int32)
        flat_counts.scatter_add_(0, indices, ones)

        grid_pts_cnt = flat_counts.view(grid_size, grid_size, grid_size)
    ...
    for bc in bc_params:
        if bc["type"] == "cuboid":
        ...
        elif bc["type"] == "add_constant_force":
            assert "force" in bc.keys()

            force =  bc["force"]
            center_point = bc["point"]
            start_time = bc["start_time"]
            end_time = bc["end_time"]
            radius = bc["radius"]
            print('sim_pos.mean: ',sim_pos.mean())
            print('sim_pos.max: ',sim_pos.max())
            print('center_pts: ',center_point)
            dist = torch.norm(sim_pos - torch.tensor(center_point).unsqueeze(0).cuda(), dim=-1)
            idx_closest = torch.argmin(dist)
            new_force = torch.tensor(force).cuda().float()
            force_info.append({"force":new_force / new_force.norm() * 0.1,"point":sim_pos[idx_closest.item()],
                               "start_frame":int(start_time/(substep_dt*substep_perframe)),
                               "end_frame":int(end_time/(substep_dt*substep_perframe))})
            ...
            if "size" in bc.keys():
                size = bc["size"]
            particle_pos = wp.to_torch(mpm_solver.mpm_state.particle_x)  # [N, 3], assumed on CUDA
            print('particle_pos.mean: ',particle_pos.mean(dim=0))
            new_point = torch.tensor(point, dtype=particle_pos.dtype, device=particle_pos.device).unsqueeze(0)  # [1, 3]
            dist = torch.norm(particle_pos - new_point, dim=-1)  # [N]
            idx_closest = torch.argmin(dist)
            new_force = torch.tensor(bc["force"]).cuda().float()
            force_info.append({"force":new_force / new_force.norm() * 0.1,"point":sim_pos[idx_closest.item()],
                               "start_frame":8*start_time,
                               "end_frame":8*start_time+8*num_dt})
