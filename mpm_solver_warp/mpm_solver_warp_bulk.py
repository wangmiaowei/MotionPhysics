...
class MPM_Simulator_WARP:
    ...
    def initialize(self, n_particles, n_grid=100, grid_lim=1.0, device="cuda:0"):
        ...
        self.mpm_model.bulk = wp.zeros(shape=n_particles, dtype=float, device=device,requires_grad=True)
        ...
        self.mpm_model.yield_stress = wp.zeros(shape=n_particles, dtype=float, device=device, requires_grad=True)
        ...
        self.mpm_state.init(n_particles, device=device,
                            n_grid=self.mpm_model.n_grid, requires_grad=True)
    ...
    def set_parameters_dict(self, kwargs={}, device="cuda:0"):
        ...
        if "mu" in kwargs:
            wp.launch(
                kernel=set_value_to_float_array,
                dim=self.n_particles,
                inputs=[self.mpm_model.mu, kwargs["mu"]],
                device=device,
            )
        ...
        if "bulk" in kwargs:
            wp.launch(
                kernel=set_value_to_float_array,
                dim=self.n_particles,
                inputs=[self.mpm_model.bulk, kwargs["bulk"]],
                device=device
            )
    ...
    def p2g2p(self, dt, device="cuda:0"):
        grid_size = (
    ...
    def set_E_nu_from_torch(self,E,nu,device="cuda:0",):
        if E.ndim == 0:
            E_inp = E.item()  # float
        else:
        ...
        self.set_E_nu(E_inp, nu_inp, device=device)

    
    def set_E_nu(self, E: float, nu: float, device="cuda:0"):
        if isinstance(E, float):
            wp.launch(
                kernel=set_value_to_float_array,
                dim=self.n_particles,
                inputs=[self.mpm_model.E, E],
                device=device,
            )
    ...
    def enforce_grid_velocity_by_mask(
        ...
        self.modify_bc.append(None)
    def add_impulse_on_particles_with_mask(self,force,particle_mask,start_time,end_time,point=[1, 1, 1],size=[1, 1, 1]):
